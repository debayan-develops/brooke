...
@section('content')
    <div id="app">
        ...
        <section class="is-hero-bar">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-6 md:space-y-0">
                ...
                <button type="button" id="btnAddCategory" class="button blue">Add Category</button>
            </div>
        </section>
        ...
        <section class="section main-section" style="padding-bottom: 0;">
            <div class="filter-section">
                ...
                <div class="filter-group">
                    ...
                    <input type="text" id="filterType" class="filter-input" placeholder="Search Type (e.g. Blog, Novel)...">
                </div>
        ...
        <section class="section main-section">
            ...
            <div class="card has-table">
                <header class="card-header">
                    <p class="card-header-title"><span class="icon"><i class="mdi mdi-square-edit-outline"></i></span> List</p>
                </header>
    ...
    <div id="add-features-modal" class="modal">
        <div class="modal-background close-modal" data-target="add-features-modal"></div>
        <div class="modal-card">
            <form method="POST" action="{{ route('admin.contentCategory.create') }}">
                @csrf
                <header class="modal-card-head">
                    <p class="modal-card-title">Add New Category</p>
                    <button type="button" class="delete close-modal" data-target="add-features-modal"></button>
                </header>
                ...
                <footer class="modal-card-foot">
                    ...
                    <button type="button" class="button close-modal" data-target="add-features-modal">Cancel</button>
                </footer>
            </form>
        </div>
    </div>
    ...
    <div id="edit-category-modal" class="modal">
        <div class="modal-background close-modal" data-target="edit-category-modal"></div>
        <div class="modal-card">
            <form id="edit-category-form" method="POST" action="">
                ...
                <header class="modal-card-head">
                    ...
                    <button type="button" class="delete close-modal" data-target="edit-category-modal"></button>
                </header>
                <section class="modal-card-body">
                    <div class="field">
                        <label class="label">Name</label>
                ...
                <footer class="modal-card-foot">
                    ...
                    <button type="button" class="button close-modal" data-target="edit-category-modal">Cancel</button>
                </footer>
            </form>
        </div>
    </div>
    ...
    <div id="delete-category-modal" class="modal">
        <div class="modal-background close-modal" data-target="delete-category-modal"></div>
        <div class="modal-card">
            <form id="delete-category-form" method="POST" action="">
                ...
                <header class="modal-card-head">
                    ...
                    <button type="button" class="delete close-modal" data-target="delete-category-modal"></button>
                </header>
                <section class="modal-card-body">
                    <p>Are you sure you want to delete <strong class="categoryName"></strong>?</p>
                </section>
                <footer class="modal-card-foot">
                    <button type="submit" class="button red">Delete</button>
                    <button type="button" class="button close-modal" data-target="delete-category-modal">Cancel</button>
                </footer>
            </form>
        </div>
    </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function renderTable(data) {
                ...
                data.forEach((item, index) => {
                    // FIX: Properly extract type names
                    let typeNames = 'None';
                    ...
                    let row = `
                        <tr>
                            ...
                            <td data-label="Type">
                                <span class="tag is-light">${typeNames}</span>
                            </td>
            ...
            function applyFilter() {
                ...
                const filtered = allCategories.filter(item => {
                    ...
                    // Type Match (Check against ALL types assigned to the category)
                    let itemTypeString = '';
                    if (item.types && item.types.length > 0) {
                    ...
                    // Both must match (if query exists)
            ...
            // 4. MODAL LOGIC
            const addChoices = new Choices('#categoryType', { removeItemButton: true });
            const editChoices = new Choices('#editCategoryType', { removeItemButton: true });
            ...
            document.getElementById('btnAddCategory').addEventListener('click', function() {
                toggleModal('add-features-modal', true);
            });
            ...
            document.addEventListener('click', function(e) {
                ...
                if (editBtn) {
                    ...
                    toggleModal('edit-category-modal', true);
                    $('#ajaxLoader').fadeIn();

                    // Setup API URLs
                    let editUrl = "{{ route('admin.contentCategory.edit', ':id') }}".replace(':id', id);
                    let updateUrl = "{{ route('admin.contentCategory.update', ':id') }}".replace(':id', id);

                    // Clear previous data
                    document.querySelector('#edit-category-form input[name="id"]').value = '';
                    document.querySelector('#edit-category-form input[name="name"]').value = '';
                    editChoices.clearStore();

                    $.get(editUrl, function (data) {
                        // Controller returns array [0 => object]
                        const cat = (Array.isArray(data)) ? data[0] : data;
                        
                        document.querySelector('#edit-category-form input[name="id"]').value = cat.id;
                        document.querySelector('#edit-category-form input[name="name"]').value = cat.name;
                        document.getElementById('edit-category-form').action = updateUrl;

                        // Populate Choices
                        const selectedIds = cat.types ? cat.types.map(t => t.id) : [];
                        ...
                        toggleModal('edit-category-modal', false);
                    });
                }
                ...
                if (deleteBtn) {
                    ...
                    const deleteUrl = "{{ route('admin.contentCategory.destroy', ':id') }}".replace(':id', id);
                    document.getElementById('delete-category-form').action = deleteUrl;
                    document.querySelector('.categoryName').innerText = name;
